...
export function TaskLine({ task, action, meta }: { task: Task; action?: React.ReactNode; meta?: React.ReactNode }) {
  return <div className="task-line"><div><b>{task.name}</b><small>{task.taskType || "runtime_task"} · {task.basePendingPoints} GP · {task.energyCost} energy</small>{meta}</div>{action}</div>;
}
