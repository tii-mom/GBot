    ...
    try {
      ...
      if (getMockMode()) {
        ...
        if (db.agent.pendingPoints < priceVal) {
          throw new Error("G budget limit reached.");
        }
  
        // Deduct legacy compatibility balance for mock fallback only
  ...
  getDropTable: async (boxId: string): Promise<{ dropTable: any[] }> => {
    try {
      ...
      if (getMockMode()) {
        ...
        return {
          dropTable: [
            { id: "drop_budget", boxProductId: boxId, assetDefinitionId: null, assetName: "G budget fallback", weight: 50, guaranteed: true, minQuantity: 1, maxQuantity: 1, rarity: "common", pointAmount: 100, energyAmount: 0, probability: 1.0 },
      ...
      if (getMockMode()) {
        ...
        return {
          ...
          steps: [
            ...
            { stepType: "settle", title: "Record evidence", description: "Creating evidence report.", requiresApproval: false, toolName: null }
          ]
        };
      }
      throw err;
      ...
      if (getMockMode()) {
        ...
        return {
          steps: [
            ...
            { id: "s8", runId, stepOrder: 8, stepType: "settle", title: "Record evidence", description: "Creating evidence report.", status: "pending" }
          ]
        };
      }
      throw err;
