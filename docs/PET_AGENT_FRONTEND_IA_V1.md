...
本规范定义了 GBot Mini App 向 Agent 养成式交互、G 燃料消耗、任务地图派遣和赏金追踪控制台升级的完整前端信息架构。

## 1. 为什么 GBot 前端要按照宠物 Agent 设计
GBot 从本质上不是一个传统外包接单网站，而是一个 **Agent Bounty Game & Execution Network**。
- 用户的心智不应该是“领单者”，而是 Agent 的策略授权者。
- Agent 具备生命周期（等级、性格、星座倾向、能量、疲劳、信任度）。
- 用户通过提供 `G` 燃料、模型能量预算 (AI Credit Budget) 与装备技能卡来培育 Agent，派它外出发现任务、执行授权工作包、提交证据，并追踪外部平台或 GBot 内部任务的验收状态。
...
- G 燃料、Token / 模型能量、用户赏金 / 待结算、今日行动次数等游戏化状态条。
...
3. **去赚钱**
   - 派遣策略设置、探索区域、统一 Opportunity 卡、需要用户确认的高风险动作。
4. **小金库**
...
## 3. 用户心智模型
用户不需要理解每个外部任务平台的复杂规则，而是 Agent 的最高策略授权者：
```
[用户 (User)] ──设定预算与安全策略──> [Policy Guard]
                                      │
                                  允许与审计
                                      ▼
[外部平台 / GBot 自有市场] <───可验证凭证与战报追踪─── [Agent]
```
...
- ExploreView 的任务卡必须展示奖励、G 消耗、Token 估算、成功率、平台、收款方式、托管说明和风险等级。
- 外部平台奖励必须标记为用户钱包或用户平台账户收款；GBot 不默认托管外部赏金。
- 高风险意图（例如跨链、外部账号提交、报价、钱包动作或平台规则不清楚的自动化）触发 `waiting_user` 状态，必须由用户亲自在 ExploreView 或 HomeView 授权。
...
- **派 Agent 赚钱**: 允许作为游戏化主 CTA，但必须配套说明外部赏金归用户、需验收、需规则允许。
- **发现任务 / 机会雷达**: 代替“确定收益入口”。
- **创造可验证价值**: 代替“必然获利”
- **用户赏金 / 待结算 / 任务方验收中**: 代替任何财务回报承诺。
