...
AND verification.source IN ('workflow_step', 'legacy_unknown')
...
// `passed` 的正式来源仅允许 workflow_step 或 legacy_unknown。
...
5. 正式有效结果只允许原 produce 或一个直接 Recovery child。第二层或更深 Recovery 永远不得被选择为最终有效结果。
6. `timed_out`、`cancelled` 当前不是合法 recover endpoint 输入；若其存在 Recovery link，报告标记 `data_inconsistent` 与受控 warning，不选择最终 execution。
7. 合法直接 Recovery 为 `completed` 时，它是最终有效 execution；未完成或失败时不存在最终有效 execution。
8. 构建展示关系必须使用 visited set、设置固定最大深度并按 execution ID 去重。
9. 发现自引用、循环、重复节点、parent/recovery 矛盾、多个 Recovery 指向同一原 produce、超过最大深度或跨 run/step/user/agent 关联时：返回 `data_incomplete`、受控 warning，且不选择最终 execution。
10. 不得把原 failed execution 改写为 completed，不得按最新 `created_at` 或最大 `attempt_number` 猜测。
11. Token 与成本必须按唯一 execution ID 去重汇总。
...
Work Report V1 不要求复杂分页，但服务端必须为 `steps`、`runtimeExecutions`、`skillUsages`、`warnings`、`sources`、`recommendations`、`fact_vs_judgment` 和所有可变字符串定义常量化硬上限：

- 查询使用稳定排序和显式 `LIMIT`。
- executions 与 usages 按 ID 去重后再计数和返回。
- 超限内容确定性截断，报告设置 `data_truncated` 并附受控 warning。
- 普通用户 API 不得返回无限数组或无限字符串。
- Admin 如未来需要完整数据，使用独立分页 endpoint，不扩大当前响应。
- 上限值须在共享类型/serializer 测试中固定，任何调整需独立审查。
...
2. 验证脚本只通过环境变量 `WORK_REPORT_API_BASE` 接收实际 origin；未设置时立即失败，不得使用不可覆盖的 `127.0.0.1:8787` 内部默认值。
...
- timed_out/cancelled 存在 Recovery link 时标记 `data_inconsistent`，不选择最终 execution。
- 第二层或更深 Recovery 不得被选为正式结果。
- 自引用、循环、重复节点、parent/recovery 矛盾、超过最大深度均返回 `data_incomplete` 与受控 warning。
- 非法多个 Recovery 被识别。
...
- 分享文本不含完整正文、原始 URL和敏感字段。
- steps、executions、skill usages、warnings、sources、recommendations、fact_vs_judgment 与所有可变字符串均受硬上限约束。
- 查询稳定排序并使用 LIMIT；超限截断后返回 data_truncated 与 warning。
- executions 与 usages 去重，普通用户 API 不返回无限数组。
...
- CI 先启动本地 Worker，health check 成功后再验证。
- 显式 `WORK_REPORT_API_BASE` 成功；未设置 base 立即失败。
- 不存在不可覆盖的 127.0.0.1:8787 内部默认值。
