...
GrowthBot 支持 Agent Bot Studio (BYO Model 接入)，但仅对白名单高级用户开放，不放在普通用户第一屏。
...
高级用户 / KOL / 项目方 / 工作室：
- 在 Pro / Studio / Project 订阅与白名单许可下，可配置自定义模型 API。
- 自定义模型 Base URL（必须匹配平台内置白名单 `agent_provider_allowlist` 且通过安全代理）、Model ID 和 API Key。
- 支持配置 Agent 名称、Prompt 模板、任务偏好、风险偏好及每日调用上限。

安全与审计约束：
- 用户 API Key 必须使用 `MODEL_CONFIG_SECRET` 加密存储，前端不得回显明文（仅展示 keyLast4）。
- 模型调用必须走后端受控代理，设置 5s 超时，32KB 限制与重定向防范，失败时安全降级至平台静态指南或默认配置。
- 日志记录必须脱敏（仅记录调用目的与成功状态），严禁在数据库审计中存储 API Key、Prompt 文本、完整 Response 或钱包地址。
- 模型输出只能作为理解任务、步骤生成、风险提示及任务推荐展示。绝对不能直接发奖、修改积分、批准验收或触发任何钱包交易。
- V1 不触碰用户主钱包，不承诺任何固定收益，自动钱包交易等特性仅作为 beta 预留，不在 V1 执行。
