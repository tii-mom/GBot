...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Runtime Catalog** | Verify runtime status of all 31 canonical skills. | `GET /skills/runtime-status returns 8 active and 23 planned skills` | `scripts/verify-skill-runtime-lite.mjs` | Returns exactly 31 skills in catalog with 8 marked as `active` and 23 as `planned`. | **PASS** |
...
| **Fresh Migration** | Verify clean database creation and seeding of active runtimes. | `Fresh DB migration and table checks (0001 -> 0014)` | `scripts/verify-skill-runtime-lite.mjs` | 1. Blank database builds from scratch. <br> 2. Tables exist, exactly 8 active runtimes are seeded. <br> 3. `PRAGMA foreign_key_check` passes. | **PASS** |
| **Upgrade Migration** | Verify database upgrade without data loss. | `0013 to 0014 Upgrade and data retention check` | `scripts/verify-skill-runtime-lite.mjs` | 1. Upgrades existing 0013 database to 0014. <br> 2. Retains all pre-upgrade user/agent/learned skill records. <br> 3. `PRAGMA foreign_key_check` passes. | **PASS** |
